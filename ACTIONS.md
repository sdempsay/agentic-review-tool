...
- Tightened `java-exceptional.md`: mandatory verification pass, explicit ban on adapter-boundary (`throws` helpers + public `try/catch`), Gson `@Override` carve-out only; synced bundled rules (Ollama parity with OpenRouter on store-fs I/O)
